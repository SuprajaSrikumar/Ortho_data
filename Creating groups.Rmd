...
```{r}
ortho_data <- read_csv(here("Data", "2025_04_02_Ortho_Data_variables_added.csv"))
```
...
ortho_data <- ortho_data %>%
  mutate(Group = case_when(
    RECORDING_SESSION_LABEL %in% td_ids ~ "TD",
    RECORDING_SESSION_LABEL %in% compensated_ids ~ "Compensated",
    RECORDING_SESSION_LABEL %in% dyslexia_ids ~ "Dyslexia",
    TRUE ~ NA_character_
  ))
...
```{r}
write_csv(ortho_data_clean, "2025_04_09_Ortho_Data_with_groups.csv")
```
