...
table_data <- table(choice_highlow$Group, choice_highlow$Response_Type)
...
#### Different ways to check if the max high ortho option chosen was correct in table 2

# 1. Based on response choice alone
high_ortho_choice_counts <- ortho_data %>%
  filter(OrthoET_response_choice == "High_Ortho") %>%
  distinct(participant_id, OrthoET_trial_number) %>%
  count(participant_id, name = "n_high_ortho_chosen")

# 2. Based on high-only accuracy variable
high_accuracy_counts <- ortho_data %>%
  filter(OrthoET_response_choice_accuracy_high == 1) %>%
  distinct(participant_id, OrthoET_trial_number) %>%
  count(participant_id, name = "n_high_accurate")

# Combine the two
combined_accuracy_counts <- full_join(
  high_ortho_choice_counts,
  high_accuracy_counts,
  by = "participant_id"
)

# Check for mismatches
combined_accuracy_counts %>%
  mutate(across(starts_with("n_"), ~replace_na(.x, 0))) %>%
  filter(n_high_ortho_chosen != n_high_accurate)
...
# Step 1: Convert RT to seconds
high_rt_data <- high_rt_data %>%
  mutate(RT_sec = OrthoET_response_time / 1000)

# Step 2: Get participant-level mean RT
participant_means_pre_winsor <- high_rt_data %>%
  group_by(participant_id, Group) %>%
  summarise(mean_RT_sec = mean(RT_sec, na.rm = TRUE), .groups = "drop")

# Step 3: Summarize participant-level means by group
rt_summary_participant_level <- participant_means_pre_winsor %>%
  group_by(Group) %>%
  summarise(
    n = n(),
...
# Step 4: Display as a table
library(gt)

rt_summary_participant_level %>%
  gt() %>%
  tab_header(
    title = "Table 3: Participant-Level Response Time Summary (Pre-Winsorization)",
    subtitle = "Each participant's mean RT (in seconds) before winsorization"
  ) %>%
  cols_label(
    Group = "Group",
    n = "n",
